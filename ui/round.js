...
stripEl.innerHTML="";for(var i=0;i<history.length;i+=1){var li=document.createElement("li");var v=history[i];var tone=v<2?"fx_chip_low":v<10?"fx_chip_mid":"fx_chip_high";li.className="fx_chip-abb46b3d"+tone;li.textContent=fmtMult(v);stripEl.appendChild(li);}}
...
bankWin(m,false);}
function blowUp(now){phase="gone";goneAt=now;round.tEnd=(now-round.t0)/1000;multEl.textContent=fmtMult(round.crashAt);multEl.classList.add("fx_mult_gone-abb46b3d");collectEl.disabled=true;pushHistory(round.crashAt);if(round.banked){say("Departed at "+fmtMult(round.crashAt)+" — you had already banked "+
...
function settleRound(){phase="idle";round=null;lockConsole(false);collectEl.disabled=true;multEl.textContent="1.00×";multEl.classList.remove("fx_mult_gone-abb46b3d");drawIdle();if(bank<1){say("Reserve is empty — press Refill to restock the demo "+"credits.","fx_down");}}
